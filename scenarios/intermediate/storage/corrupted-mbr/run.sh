...
The MBR partition table has been corrupted, and the operating system cannot find the partition you created or the files inside it.
Something may have gone wrong on the disk, but the system can still detect the disk device and read it.
Check what is wrong, fix the issue, and read the files inside the recovered partitions.
It is highly recommended that you research and learn about the following concepts before starting:

1.MBR Partition Table
2.How Linux deals with disks and partitions
3.Partition recovery tools and techniques
4.parted command
5.fdisk command
6.testdisk command
7.mount command
...
parted -s "$DISK" mkpart primary ext4 2GiB 4GiB
...
dd if=/dev/zero of="$DISK" bs=1 seek=446 count=64 
